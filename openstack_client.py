...
# Mapping VM → fichier YAML avec le bon nodeSelector (PoP label)
# space_1 → edge1, cloud1  |  space_2 → edge2, cloud2
...
def _apply_stream_source(cluster: str, to_vm: str) -> bool:
    """Applique tc-stream-source sur le cluster cible avec le bon nodeSelector."""
    yaml_file = YAML_PER_VM.get(to_vm, "tc-stream-source-cloud.yaml")
    log.info(
        f"YAML sélectionné pour {to_vm}: {yaml_file}",
        extra={"event": "yaml_selected"}
    )
    try:
        result = _run_kubectl([
            "--context", cluster,
            "apply", "-f", f"{STAGE_DIR}/{yaml_file}",
            "-n", K8S_NAMESPACE
        ])
        if result.returncode == 0:
            log.info(f"Stream source applied on {cluster} (yaml={yaml_file})",
    ...
    def do_POST(self):
        if self.path == "/migrate":
            try:
                ...
                yaml_file = YAML_PER_VM.get(to_vm, "tc-stream-source-cloud.yaml")
                log.info(
                    f"Migration {from_vm}({from_cluster}) → {to_vm}({to_cluster}) | yaml={yaml_file}",
                    extra={"event": "migration_requested"}
                )

                # Supprimer sur la source
                if from_cluster:
                ...
                # Appliquer sur la cible avec le bon YAML
                success = _apply_stream_source(to_cluster, to_vm)
                ...
                log.info(
                    f"Migration complete → {to_vm} ({to_cluster}) | yaml={yaml_file}",
...
if __name__ == "__main__":
    ...
    log.info(f"yaml_map={YAML_PER_VM}", extra={"event": "startup"})
