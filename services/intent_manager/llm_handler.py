...
class LLMHandler:
    ...
    async def _level1_llm(self, text: str, context: Dict[str, Any]) -> Optional[List[Dict[str, Any]]]:
        ...
        system_prompt = (
            ...
            "intention centrée sur la réactivité ou la disponibilité peut ne "
            "produire QUE latency. Ne complète JAMAIS à 3 par habitude.\n\n"
            "FORMAT DE RÉPONSE OBLIGATOIRE — objet JSON uniquement, "
            "sans texte avant ou après, sans markdown.\n\n"
            "EXEMPLES (le nombre de SLOs VARIE selon l'intention — n'en produis pas "
            "3 par réflexe) :\n\n"
            "Intention : \"préviens-moi tout de suite si le site tombe\"\n"
            "→ {\"merge_strategy\":\"replace\",\"slos\":["
            "{\"metric\":\"latency\",\"operator\":\"<\",\"threshold\":80.0,\"unit\":\"ms\","
            "\"weight\":1.0,\"target\":72.0,\"window\":\"5m\"}]}\n"
            "(une alerte = réactivité pure → latence SEULE)\n\n"
            ...
            "{\"metric\":\"ram_usage\",\"operator\":\">=\",\"threshold\":1.5,\"unit\":\"GB\","
            "\"weight\":0.4,\"target\":1.7,\"window\":\"5m\"}]}\n"
            "(calcul lourd, pas d'exigence de latence explicite → CPU + RAM, PAS de "
            "latence)\n\n"
            "Intention : \"streaming vidéo fluide et sans latence\"\n"
            "→ {\"merge_strategy\":\"replace\",\"slos\":["
            "{\"metric\":\"latency\",\"operator\":\"<\",\"threshold\":100.0,\"unit\":\"ms\","
            "\"weight\":0.5,\"target\":90.0,\"window\":\"5m\"},"
            "{\"metric\":\"cpu_usage\",\"operator\":\">=\",\"threshold\":1.5,\"unit\":\"cores\","
            "\"weight\":0.3,\"target\":1.7,\"window\":\"5m\"},"
            "{\"metric\":\"ram_usage\",\"operator\":\">=\",\"threshold\":1.0,\"unit\":\"GB\","
            "\"weight\":0.2,\"target\":1.2,\"window\":\"5m\"}]}\n"
            "(fluidité = CPU/RAM + latence → les 3)"
        )
